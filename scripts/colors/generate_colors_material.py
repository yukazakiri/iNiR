...
if args.termscheme is not None:
    ...
    for color, val in term_source_colors.items():
        ...
        # term8 (bright black): Make visible against background
        # Dark mode: needs to be lighter than background for contrast
        # Light mode: needs to be darker than background for contrast
        if color == "term8":
            if darkmode:
                term_colors[color] = get_interpolated_surface(0.35)
            else:
                term_colors[color] = get_interpolated_surface(0.25)
            continue

        # term7 (white/foreground): Ensure visibility in both modes
        if color == "term7":
            if darkmode:
                term_colors[color] = get_interpolated_surface(0.55)
            else:
                term_colors[color] = get_interpolated_surface(0.40)
            continue
