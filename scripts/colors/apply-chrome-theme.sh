...
# Clears user themes, sets both color_scheme and color_scheme2 explicitly.
...
fix_preferences() {
  local prefs_dir="$1"
  local name="$2"
  local cs2="$3"  # color_scheme: 2=dark, 1=light
  ...
  # browser.theme.color_scheme         → 2=dark, 1=light (explicit, no portal needed)
...
apply_to_browser() {
  ...
  # Map mode to Chromium's internal integers for Preferences
  # 2 = Dark, 1 = Light
  local pref_cs2=1
  if [[ "$mode" == "dark" ]]; then
    pref_cs2=2
